...
const Tab3: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const data = await fetchPronXUsuario();
        if (data && data.length > 0 && data[0].espagado === false) {
          setShowToast(true);
        }
      } catch (err) {
  ...
  const posiciones = [...(pronosticosConAciertos ?? [])].sort(
      ...
      <IonContent fullscreen>
        ...
        <IonCard>
         ...
         <IonToast
              ...
              message="⚠️ Aviso importante ⚠️
Recuerda que si tu pronóstico no está pagado, no aparecerá en la lista de participantes.
Asegúrate de completar tu pago por transferencia para que tu pronóstico quede registrado ✅."
